...
% 
%**************************************************************************
%% load path for appropriate toolboxes to use
...
cd ~/Dropbox/ProjectSurveyDissapationRange/data/Interval_01
load C3_CIS_20060307_1400_1432.mat;
...
% cd /home/khurom/Dropbox/work.plasma/ProjectSurveyDissapationRange/data/Interval_17
...
load C3_BTot_20060307_1400_1432.mat   % Interval_01/home
...
% cd /home/khurom/Dropbox/work.plasma/ToolBox/

%% load Cassini interval data 2004-06-28

clear all; close all;

cd('/Users/khurom/Desktop/LinaCassiniData/2004-06-28');

load 2004-06-28--cassini_proton_Velocity.mat;

vel=nanmean(velocity(:,2:4),1);
...
load Time2004062806301800.mat;  % 06:30:00 --> 18:00:00

startdata=748800; enddata=2073600;

BTot=data(startdata:enddata,:);

clear data startdata enddata

Bmag=sqrt(BTot(:,2).*BTot(:,2) + BTot(:,3).*BTot(:,3) + BTot(:,4).*BTot(:,4));
Bmag=mean(Bmag);

%% load Cassini interval data 2007-03-12 


clear all; close all;

cd('/Users/khurom/Desktop/LinaCassiniData/2007-03-12');

load 2007-03-12--cassini_proton_Velocity.mat;
...
load Time200703120104.mat;  % 06:30:00 --> 18:00:00

startdata=116130; enddata=464500;
...
%% Putting SWT_parts into a different cell structure to facilitate
%  calculation of moments and kurtosis later on of the components withhout
%  the anisotropic decomposition.

cmpntDetails=cell(1,4);

cmpntDetails{1,1}=SWT_parts{1,1};

for m=2:1:4

cmpntDetails{1,m}=cell(1,nlevel);

for i=1:1:nlevel

    cmpntDetails{1,m}{1,i}=SWT_parts{1,m}{1,2}(i,:);
...
%% Calculate Kurtosis of the different components

% [xTimeScales,xkurt]=kurt(cmpntDetails{1,2},scale.*dt,1);
% [yTimeScales,ykurt]=kurt(cmpntDetails{1,3},scale.*dt,1);
[zTimeScales,zkurt]=kurt(cmpntDetails{1,4},scale.*dt,1);
...
for k = 1:nlevel
    ...
    dmagcom = dmagcom./(sqrt(lengthofsamples));
...
%% plotting PSD
    ...
    plot(log10(frequency(1:end)),log10(p_par(1:end)),'bo-','LineWidth',2,...
...
%     errorbar(log10(frequency),log10(p_perpB),dp_perpB,'k');
    ...
    plot(log10(frequency(1:end)),log10(p_perpMag(1:end)),'ro-','LineWidth',2,...
...
%     axis square
  ...
  plot(log10(frequency(1:end)),log10(p_par(1:end)./p_perpMag(1:end)),...
...
%% Magnetic Compressibility plot

figure(2)
    
%     errorbar(log10(krhoi(:)),log10(magcom(:)),dmagcom(:),'k');
...
%     xlabel('k\rho_{i}')
    xlabel('spacecraft frequency (Hz)')
    ylabel('C_{||}')
...
%% Structure functions
tic
[tausPar,AllMomentsPar]=swfunctions(coefspar,dt,scale,1);
toc
% tic
% [tausPerp,AllMomentsPerp]=swfunctions(coefsperpMag,dt,scale,1);
% toc
...
for k=4:1:10
    ...
    NormHisto(coefspar{1,k},200,0,1,'-b'); hold on;
...
hist(coefsBVangle{1,12},50);
nanmean(coefsBVangle{1,12})
