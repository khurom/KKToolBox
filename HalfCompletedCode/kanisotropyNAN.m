%**************************************************************************
% m-file for project: Solar wind turbulence k-anisotropy study
%**************************************************************************
% 
% File name: kanisotropy.m
% Author: K. H. Kiyani
...
% 
% 3./   Whether one takes half the size of the filter or the full size
%       doesn't really seem to matter, strangely. Also I looked at
%       constructing the directions directly from the approximation
%       coefficients and these don;t seem to change so much from the ones
%       given by the reconstructed approximations. So it might be worth
%       constructing a background field from the approximation coefficients
%       without the reconstruction as you lose less sample points due to
%       edge effects. Although the older scheme makes more sense to me.
% 
%% load ULYSSES data (below 2 second interpolation)

clear all; close all;

% cd('~/Dropbox/work.plasma/ProjectLocalAnisotropy/Data/Ulysses');
cd('C:\Dropbox\work.plasma\ProjectLocalAnisotropy\Data\Ulysses\');
load UY_1SEC_VHM_29502.mat;

threshold=2; tolerance=0.2;

data=RegulariseGrid2(UY_1SEC_VHM_29502,threshold,tolerance);

clear('threshold','tolerance','UY_1SEC_VHM_29502');

BTot=[data(:,1),data(:,3:5)];
Bmag=[data(:,1),data(:,2)];
...
load UY_M0_BAI_31763.mat;

vel=nanmean(UY_M0_BAI_31763(:,4:6),1);

clear('UY_M0_BAI_31763');

velmag=norm(vel);
 
vel=vel./norm(vel);

% This stuff is needed to calculate rho_i
% 
% Temperature=load('Temperature_from_somewhere');
% 
% ionTperp=mean(Temperature);
% 
% clear('Temperature');

%% load ULYSSES data (everything interpolated)

clear all; close all;

% cd('/Users/khurom/Dropbox/work.plasma/ProjectLocalAnisotropy/Data/Ulysses');
cd('C:\Dropbox\work.plasma\ProjectLocalAnisotropy\Data\Ulysses\');

load UY_1SEC_VHM_29502.mat;

in=UY_1SEC_VHM_29502;
clear('UY_1SEC_VHM_29502');
...
if (2^(nextpow2(samplelength))) - 1 < samplelength; 
    BTot=BTot(1:end-2,:);
    samplelength=length(BTot(:,1));
end

% I don't want the length of the signal to equal 2^integer
% otherwise there is no NaN at the beginning and the end of the signal
% These are neccessary to take into account edge effets.

dt=1;   % ULYSSES sampling rate
...
% sample with NaNs
    ...
    y=[nan(pads/2,1); BTot(:,m); nan(pads/2,1)];
...
%% plotting PSD
    ...
    plot(log10(frequency(1:end)),(p_par(1:end)),'bo-','LineWidth',2,...
...
%     errorbar(log10(frequency),log10(p_perpB),dp_perpB,'k');
    ...
    plot(log10(frequency(1:end)),(p_perpMag(1:end)),'ro-','LineWidth',2,...
